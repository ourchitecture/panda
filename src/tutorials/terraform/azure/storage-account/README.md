...
7. Test the terraform setup by typing `make check`. You should see output similar to the below.
...
8. Review the name prefix and other variables in the file "./tutorial.tfvars". Change the values as needed. The resource group name should already exist. To get a list of existing resource groups type `make list-resource-groups`.

9. The [terraform workflow][tf-workflow] encourages us to plan our changes and then to apply them. Consider the name and location for your new Resource Group. Then, create a plan by typing `make plan-install-var-file`.

10. Review the plan created by terraform. Ignore the "random" resources used to create random numbers, but pay special attention to how the "storage_account" and container will be created.

11. Now that a plan exists, execute the plan and create the resources by typing `make install`.

12. Verify the resource has been created by typing `make list-storage-accounts resource-group="our-tutorials-rg"` where "our-tutorials-rg" is the name of the resource group and checking if the new storage account exists in the list.

13. To remove the new storage account, first type `make plan-uninstall-var-file`. If you want to see what will be destroyed prior to executing this command, type `make uninstall`.
