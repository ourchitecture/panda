...
9. Create the AppService by typing `make install ENV_FILE=.env`. If you want to see what will be created prior to executing this command, type `make install TF_PLAN_ONLY=true ENV_FILE=.env`.
...
11. To remove the new AppService, type `make uninstall ENV_FILE=.env`. If you want to see what will be destroyed prior to executing this command, type `make uninstall TF_PLAN_ONLY=true ENV_FILE=.env`.
