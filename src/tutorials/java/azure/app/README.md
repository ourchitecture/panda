...
- A [Microsoft Azure account][azure-account] with an existing App Service Plan and permission to create and deploy an AppService (this Node.js application) to the plan. _\*See the tutorials to [create a new resource group][tutorial-rg] and [create a Java App Service][tutorial-app-service-plan]._
- [git][git]
- [make][make]
- [docker][docker]
...
12. Deploy the application to Azure by typing `make install`. You should see output similar to the below:
...
14. To remove the new application, type `make uninstall`.
